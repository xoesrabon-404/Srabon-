...
const fs = global.nodemodule["fs-extra"];
 ...
 let name = await Users.getNameUser(event.messageReply.senderID) 
	...
	var callback = () => api.sendMessage({body:`༺ᜊ𝐽𝑖ℎ𝑎𝑑 𝐻𝑎𝑠𝑎𝑛ツ༻`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
 ...
 var callback = () => api.sendMessage({body:`༺ᜊ𝐽𝑖ℎ𝑎𝑑 ⏤͟͟͞͞𝐶ℎ𝑎𝑡 ⏤͟͟͞͞𝐵𝑜𝑡ツ༻`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
 () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
...
var data = await api.getUserInfoV2(res_ID);
 ...
 var link = data.link
 var callback = () => api.sendMessage({body:`༺ᰔ⏤͟͟͞͞𝐽𝑖ℎ𝑎𝑑 ⏤͟͟͞͞𝑐ℎ𝑎𝑡 ⏤͟͟͟͟͞͞͞͞𝑏𝑜𝑡ツ༻`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
 () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
 ...
 var callback = () => 
api.sendMessage({body:`༺☹︎☯𝐽𝑖ℎ𝑎𝑑 𝑐ℎ𝑎𝑡 𝑏𝑜𝑡࿐ッ༻`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
 () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
 return request(encodeURI(`https://graph.facebook.com/${uid}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',
 () => callback()); 
