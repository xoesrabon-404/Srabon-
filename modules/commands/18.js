module.exports.config = {
    ...
    hasPermssion: 2,
    credits: "Jihad",
    description: "Play 19+ content from provided links",
...
module.exports.run = async function({ api, event }) {
    try {
        if (link.length === 0) return api.sendMessage("💔 কোনো লিংক পাওয়া যায়নি।", event.threadID);

        // এলোমেলো লিংক নির্বাচন
        const randomLink = link[Math.floor(Math.random() * link.length)];

        // ডাউনলোড করার জন্য ফাইলের নাম
        const fileName = path.join(__dirname, "temp.mp4");

        // লিংক থেকে ফাইল ডাউনলোড করা
        ...
        console.error(error);
        api.sendMessage("💔 কোনো ভুল হয়েছে।", event.threadID);
    }
};
